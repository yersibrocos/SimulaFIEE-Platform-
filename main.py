...
def draw_title(screen, font):
    shadow = font.render("Simulacion Educativa FIEE", True, (5, 8, 12))
    title = font.render("Simulacion Educativa FIEE", True, (247, 252, 255))

    shadow_rect = shadow.get_rect(center=(WINDOW_SIZE[0] // 2 + 2, 105 + 2))
    title_rect = title.get_rect(center=(WINDOW_SIZE[0] // 2, 105))
    screen.blit(shadow, shadow_rect)
    screen.blit(title, title_rect)


def fade_transition(screen, clock, current_surface, target_surface, duration_ms):
...
def launch_external_script(base_path, script_name):
    script_path = base_path / script_name
    if not script_path.exists():
        return

    subprocess.run([sys.executable, str(script_path)], cwd=str(base_path), check=False)


def main():
    pygame.init()
    pygame.display.set_caption("FIEE - Inicio")
    ...
    options_font = pygame.font.SysFont("Segoe UI", 28)
    ...
    options_button = TextButton((x, first_y + button_h + gap, button_w, button_h), "Opciones", button_font)
    exit_button = TextButton((x, first_y + (button_h + gap) * 2, button_w, button_h), "Salir", button_font)
    ...
    options_back = ArrowButton((40, WINDOW_SIZE[1] - 110, 220, 70), "left", "Volver", arrow_font)
    ...
    cyber_workshop_modules = build_placeholder_modules(["Proteccion del servidor", "En desarrollo"])
    ...
    while running:
        ...
        if state == STATE_MENU:
            for button in (explore_button, options_button, exit_button):
            ...
            options_back.update_hover(mouse_pos)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                elif state in (STATE_EXPLORE_1, STATE_OPTIONS):
                ...
                if target_state:
                    ...
                    fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                    state = target_state
                    current_background = target_background

            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if state == STATE_MENU:
                    if explore_button.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_1
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                    ...
                    if options_back.is_clicked(event.pos):
                        state = STATE_MENU

                elif state == STATE_EXPLORE_1:
                    if left_back.is_clicked(event.pos):
                        target_state = STATE_MENU
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif right_next.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_2
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                    ...
                    if right_back.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_1
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif workshop_electric.is_clicked(event.pos):
                        target_state = STATE_WORKSHOP_ELECTRIC
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif workshop_electronics.is_clicked(event.pos):
                        target_state = STATE_WORKSHOP_ELECTRONICS
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif workshop_telecom.is_clicked(event.pos):
                        target_state = STATE_WORKSHOP_TELECOM
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif workshop_cyber.is_clicked(event.pos):
                    ...
                    if electric_workshop_back.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_2
                        target_background = backgrounds[target_state]
                        fade_transition(screen, clock, current_background, target_background, TRANSITION_MS)
                        state = target_state
                        current_background = target_background
                    elif electric_workshop_modules[0].is_clicked(event.pos):
                        launch_external_script(base_path, "electrica.py")
                    elif electric_workshop_modules[1].is_clicked(event.pos):
                        launch_external_script(base_path, "maquina electrica.py")
                    elif electric_workshop_modules[2].is_clicked(event.pos):
                        launch_external_script(base_path, "control_industrial.py")
                    elif electric_workshop_modules[3].is_clicked(event.pos):
                        launch_external_script(base_path, "energia_solar.py")
                    elif electric_workshop_modules[4].is_clicked(event.pos):
                        launch_external_script(base_path, "circuito electrico.py")
                elif state == STATE_WORKSHOP_ELECTRONICS:
                    if electric_workshop_back.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_2
                        target_background = backgrounds[target_state]
                        ...
                        state = target_state
                        current_background = target_background
                    elif electronics_workshop_modules[0].is_clicked(event.pos):
                        launch_external_script(base_path, "mainAdvComp.py")
                elif state == STATE_WORKSHOP_TELECOM:
                    if electric_workshop_back.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_2
                        target_background = backgrounds[target_state]
                        ...
                        state = target_state
                        current_background = target_background
                    elif telecom_workshop_modules[0].is_clicked(event.pos):
                        launch_external_script(base_path, "teleco.py")
                elif state == STATE_WORKSHOP_CYBER:
                    if electric_workshop_back.is_clicked(event.pos):
                        target_state = STATE_EXPLORE_2
                        target_background = backgrounds[target_state]
                        ...
                        state = target_state
                        current_background = target_background
                    elif cyber_workshop_modules[0].is_clicked(event.pos):
                        launch_external_script(base_path, "ciber.py")
        ...
        if state == STATE_MENU:
            ...
            overlay = pygame.Surface(WINDOW_SIZE, pygame.SRCALPHA)
            overlay.fill((2, 12, 24, 150))
            screen.blit(overlay, (0, 0))

            panel_rect = pygame.Rect(WINDOW_SIZE[0] // 2 - 340, WINDOW_SIZE[1] // 2 - 170, 680, 280)
            pygame.draw.rect(screen, (12, 30, 53), panel_rect, border_radius=16)
            pygame.draw.rect(screen, (223, 238, 255), panel_rect, width=2, border_radius=16)

            title = button_font.render("Opciones", True, (245, 251, 255))
            line1 = options_font.render("Seccion en desarrollo para la simulacion educativa.", True, (235, 245, 255))
            line2 = info_font.render("Presiona Volver o ESC para regresar al menu.", True, (225, 238, 255))

            screen.blit(title, title.get_rect(center=(WINDOW_SIZE[0] // 2, panel_rect.y + 58)))
            screen.blit(line1, line1.get_rect(center=(WINDOW_SIZE[0] // 2, panel_rect.y + 128)))
            screen.blit(line2, line2.get_rect(center=(WINDOW_SIZE[0] // 2, panel_rect.y + 184)))
